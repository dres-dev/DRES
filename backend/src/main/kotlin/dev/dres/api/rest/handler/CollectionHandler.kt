...
abstract class CollectionHandler(
    protected val collections: DAO<MediaCollection>,
    protected val items: DAO<MediaItem>
) : RestHandler, AccessManagedRestHandler {
    ...
    private fun collectionById(id: UID, ctx: Context): MediaCollection =
        collections[id] ?: throw ErrorStatusException(
            404,
    ...
    protected fun collectionFromContext(ctx: Context): MediaCollection =
        collectionById(collectionId(ctx), ctx)

}

class ListCollectionHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), GetRestHandler<List<RestMediaCollection>> {
    ...
    override fun doGet(ctx: Context) =
        this.collections.map { RestMediaCollection.fromMediaCollection(it) }

    override val route: String = "collection/list"
}

class AddCollectionHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), PostRestHandler<SuccessStatus> {
    ...
    override fun doPost(ctx: Context): SuccessStatus {
        ...
        if (restCollection.basePath == null) {
            throw ErrorStatusException(
                400,
                "Invalid parameters, collection base path not set.",
                ctx
            )
        }
        ...
        if (collections.find { it.name == restCollection.name } != null) {
            throw ErrorStatusException(
                400,
                "Invalid parameters, collection with name ${restCollection.name} already exists.",
                ctx
            )
        }
        ...
        val collection = MediaCollection(
            UID.EMPTY,
            restCollection.name,
            restCollection.description,
            restCollection.basePath.cleanPathString()
        )
...
class DeleteCollectionHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), DeleteRestHandler<SuccessStatus> {
...
class UpdateCollectionHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), PatchRestHandler<SuccessStatus> {
    ...
    override fun doPatch(ctx: Context): SuccessStatus {
        ...
        val collection = collections[restCollection.id.UID()]
            ?: throw ErrorStatusException(
                400,
                "Invalid parameters, collection with ID ${restCollection.id} does not exist.",
                ctx
            )

        val updatedCollection = MediaCollection(
            collection.id,
            restCollection.name.trim(),
            restCollection.description ?: collection.description,
            restCollection.basePath?.cleanPathString() ?: collection.basePath
        )
...
class ShowCollectionHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), GetRestHandler<RestFullMediaCollection> {
    ...
    override fun doGet(ctx: Context): RestFullMediaCollection {
        ...
        val items =
            items.filter { it.collection == collection.id }.map { RestMediaItem.fromMediaItem(it) }
...
class AddMediaItemHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), PostRestHandler<SuccessStatus> {
    ...
    override fun doPost(ctx: Context): SuccessStatus {
        ...
        val existing =
            items.find { it.collection == collectionId && it.name.trim() == mediaItem.name.trim() }
        if (existing != null) {
            throw ErrorStatusException(
                400,
                "item with name '${mediaItem.name}' already exists in collection: $existing",
                ctx
            )
        }
...
class UpdateMediaItemHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), PatchRestHandler<SuccessStatus> {
    ...
    override fun doPatch(ctx: Context): SuccessStatus {
        ...
        items[mediaItem.id.UID()] ?: throw ErrorStatusException(
            400,
            "item with name '${mediaItem.name}' does not exists",
            ctx
        )
...
class GetMediaItemHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), GetRestHandler<RestMediaItem> {
    ...
    override fun doGet(ctx: Context): RestMediaItem {
        ...
        val item = this.items[mediaId] ?: throw ErrorStatusException(
            404,
...
class DeleteMediaItemHandler(collections: DAO<MediaCollection>, items: DAO<MediaItem>) :
    CollectionHandler(collections, items), DeleteRestHandler<SuccessStatus> {
...
class ResolveMediaItemListByNameHandler(
    collections: DAO<MediaCollection>,
    items: DAO<MediaItem>,
    private val mediaItemCollectionNameIndex: DaoIndexer<MediaItem, Pair<UID, String>>
) : CollectionHandler(collections, items), PostRestHandler<List<RestMediaItem>> {
    ...
    override fun doPost(ctx: Context): List<RestMediaItem> {
        val queriedNames = try {
            ctx.bodyAsClass(Array<String>::class.java)
        } catch (e: BadRequestResponse) {
            throw ErrorStatusException(
                400,
                "Invalid parameters: Expected Array<String> - This is a programmers error!",
                ctx
            )
        }
        val collection = collectionFromContext(ctx)
        val names = mediaItemCollectionNameIndex.keys().filter { it.first == collection.id }
        return if (!queriedNames.isEmpty()) {
            names.filter { queriedNames.contains(it.second) }
                .map { RestMediaItem.fromMediaItem(mediaItemCollectionNameIndex[it].first()) }
        } else {
            emptyList()
        }
    }
...
class RandomMediaItemHandler(
    collections: DAO<MediaCollection>,
    items: DAO<MediaItem>,
    private val mediaItemCollectionUidIndex: DaoIndexer<MediaItem, Pair<UID, UID>>
) : CollectionHandler(collections, items), GetRestHandler<RestMediaItem> {

    private val rand =
        Random(System.currentTimeMillis()) // TODO Decide upon seed -- time based or fixed?
