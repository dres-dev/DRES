            ...
            do {
                ...
                when (next.second.answers().firstOrNull()?.type()) {
                    ...
                    AnswerType.ITEM -> {
                        ...
                        return@transactional ApiJudgementRequest(next.first, item.type().toApi(), validator.id, item.collection.id, item.mediaItemId, taskDescription, null, null)
                    }
                    AnswerType.TEMPORAL -> {
                        ...
                        return@transactional ApiJudgementRequest(next.first, item.type().toApi(), validator.id, item.collection.id, item.mediaItemId, taskDescription, start, end)
                    }
                    else -> continue
                }
            } while (true)
