...
class RestCompetitionStartAuditLogEntry(id: UID, val competition: UID, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.COMPETITION_START, id) {
    constructor(competition: UID, api: LogEventSource, user: String?) : this(UID.EMPTY, competition, api, user)
    constructor(log: CompetitionStartAuditLogEntry) : this(log.id, log.competition, log.api, log.user)
}

class RestCompetitionEndAuditLogEntry(id: UID, val competition: UID, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.COMPETITION_END, id) {
    constructor(competition: UID, api: LogEventSource, user: String?) : this(UID.EMPTY, competition, api, user)
    constructor(log: CompetitionEndAuditLogEntry) : this(log.id, log.competition, log.api, log.user)
}

class RestTaskStartAuditLogEntry(id: UID, val competition: UID, val taskName: String, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.TASK_START, id) {
    constructor(competition: UID, taskName: String, api: LogEventSource, user: String?) : this(UID.EMPTY, competition, taskName, api, user)
    constructor(log: TaskStartAuditLogEntry) : this(log.id, log.competition, log.taskName, log.api, log.user)
}

class RestTaskModifiedAuditLogEntry(id: UID, val competition: UID, val taskName: String, val modification: String, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.TASK_MODIFIED, id) {
    ...
    constructor(log: TaskModifiedAuditLogEntry) : this(log.id, log.competition, log.taskName, log.modification, log.api, log.user)
}

class RestTaskEndAuditLogEntry(id: UID, val competition: UID, val taskName: String, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.TASK_END, id) {
    constructor(competition: UID, taskName: String, api: LogEventSource, user: String?) : this(UID.EMPTY, competition, taskName, api, user)
...
class RestJudgementAuditLogEntry(id: UID, val competition: UID, val validator: String, val token: String, val verdict: SubmissionStatus, val api: LogEventSource, val user: String?) : RestAuditLogEntry(AuditLogEntryType.JUDGEMENT, id) {
    ...
    constructor(log: JudgementAuditLogEntry) : this(log.id, log.competition, log.validator, log.token, log.verdict, log.api, log.user)
}
