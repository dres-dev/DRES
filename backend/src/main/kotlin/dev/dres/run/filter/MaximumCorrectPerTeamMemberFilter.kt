...
/**
 * A [SubmissionFilter] that checks that the maximum number of correct submissions has not been exceeded for the submitting team member and task.
 *
 * @author Ralph Gasser
 ...
 * @version 2.0.0
 */
class MaximumCorrectPerTeamMemberFilter(private val limit: Int = PARAMETER_KEY_LIMIT_DEFAULT) : AbstractSubmissionFilter("Maximum number of correct submissions ($limit) exceeded for the team member.") {
    companion object {
    ...
    override fun test(t: ApiClientSubmission): Boolean {
        ...
        require(t.userId != null) { "Submission ${t.submissionId} is not associated with a user. This is a programmer's error!" }
        ...
        for (answerSet in t.answerSets) {
            ...
            if (task.answerSets.filter { (it.status eq DbVerdictStatus.CORRECT) and (it.submission.team.id eq t.teamId) and (it.submission.user.id eq t.userId) }.size() >= this.limit) {
                return false
            }
        }
        return true
