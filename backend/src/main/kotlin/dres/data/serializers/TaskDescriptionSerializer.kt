...
object TaskDescriptionSerializer: Serializer<TaskDescription> {
    ...
    override fun deserialize(input: DataInput2, available: Int): TaskDescription = when (input.readInt()) {
        TaskType.KIS_VISUAL.ordinal -> TaskDescription.KisVisualTaskDescription(MediaItemSerializer.deserialize(input, available) as MediaItem.VideoItem, TemporalRangeSerializer.deserialize(input, available))
        TaskType.KIS_TEXTUAL.ordinal -> TaskDescription.KisTextualTaskDescription(MediaItemSerializer.deserialize(input, available) as MediaItem.VideoItem, TemporalRangeSerializer.deserialize(input, available), (0 until input.readInt()).map { input.readUTF() }, input.readInt())
        TaskType.AVS.ordinal -> TaskDescription.AvsTaskDescription(input.readUTF())
