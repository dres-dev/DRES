...
class KisTaskScorerTest {
    ...
    private val dummyImageItems = listOf(DbMediaItem.ImageItem(EvaluationId(), "image 1", "images/1", EvaluationId()))
    ...
    fun immediatelyRight() {
        val taskStartTime = System.currentTimeMillis() - 100_000
        val submissions = listOf(
                DbSubmission.Item(teams[0], EvaluationId(), taskStartTime, dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT }
        )
        val scores = this.scorer.computeScores(submissions, TaskContext(teams, taskStartTime, defaultTaskDuration))
    ...
    fun rightAtTheEnd() {
        val taskStartTime = System.currentTimeMillis() - 100_000
        val submissions = listOf(
                DbSubmission.Item(teams[0], EvaluationId(), taskStartTime + (defaultTaskDuration * 1000), dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT }
        )
        val scores = this.scorer.computeScores(submissions, TaskContext(teams, taskStartTime, defaultTaskDuration))
    ...
    fun rightInTheMiddle() {
        val taskStartTime = System.currentTimeMillis() - 100_000
        val submissions = listOf(
                DbSubmission.Item(teams[0], EvaluationId(), taskStartTime + (defaultTaskDuration * 1000 / 2), dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT }
        )
        val scores = this.scorer.computeScores(submissions, TaskContext(teams, taskStartTime, defaultTaskDuration))
    ...
    fun wrongSubmissionPenalty() {
        ...
        val submissions = listOf(
                ...
                DbSubmission.Item(teams[0], EvaluationId(), taskStartTime + 1, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },

                DbSubmission.Item(teams[1], EvaluationId(), taskStartTime + 2, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },
                DbSubmission.Item(teams[1], EvaluationId(), taskStartTime + 3, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },

                DbSubmission.Item(teams[2], EvaluationId(), taskStartTime + 4, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },
                DbSubmission.Item(teams[2], EvaluationId(), taskStartTime + 5, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },
                DbSubmission.Item(teams[2], EvaluationId(), taskStartTime + 6, dummyImageItems[0]).also { it.status = DbVerdictStatus.WRONG },

                //correct submissions at 1/2 the task time
                DbSubmission.Item(teams[0], EvaluationId(), taskStartTime + (defaultTaskDuration * 1000 / 2), dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT },
                DbSubmission.Item(teams[1], EvaluationId(), taskStartTime + (defaultTaskDuration * 1000 / 2), dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT },
                DbSubmission.Item(teams[2], EvaluationId(), taskStartTime + (defaultTaskDuration * 1000 / 2), dummyImageItems[0]).also { it.status = DbVerdictStatus.CORRECT },
        )
        val scores = this.scorer.computeScores(submissions, TaskContext(teams, taskStartTime, defaultTaskDuration))
