...
import {
  ...
  RestMediaCollection, RestMediaItem,
...
import {CompetitionFormBuilder} from '../../competition-builder-task-dialog/competition-form.builder';
import {CompetitionBuilderTaskDialogData} from '../../competition-builder-task-dialog/competition-builder-task-dialog.component';
import {FormControl, FormGroup} from '@angular/forms';
import {Observable} from 'rxjs';
import {
  VideoPlayerSegmentBuilderData
} from '../../competition-builder-task-dialog/video-player-segment-builder/video-player-segment-builder.component';
...
export class EditableTaskComponent implements OnInit {
  ...
  uploaded = (taskData: string) => {
    ...
    this.builder = new CompetitionFormBuilder(this.taskGroup, this.taskType, this.collectionService, task);
    this.form = this.builder.form;
  ...
  public pickRandomMediaItem(collectionId: string, target: FormControl) {
    this.collectionService
        .getApiV1CollectionWithCollectionidRandom(collectionId)
        .pipe(first())
        .subscribe((value) => {
  ...
  public pickRandomSegment(item: RestMediaItem, startControl: FormControl, endControl: FormControl, unitControl: FormControl) {
    const start = EditableTaskComponent.randInt(1, item.durationMs / 1000 / 2); // always in first half
    let end = 1;
    do {
      end = start + EditableTaskComponent.randInt(5, item.durationMs / 1000); // Arbitrary 5 seconds minimal length
  ...
  toggleVideoPlayer(mediaItem: RestMediaItem, startControl?: FormControl, endControl?: FormControl, unitControl?: FormControl) {
    ...
    if (startControl && startControl.value) {
      if (unitControl.value === 'TIMECODE') {
        ...
        start =
            TimeUtilities.point2Milliseconds({ value: startControl.value, unit } as RestTemporalPoint, mediaItem.fps) / 1000;
      }
