...
export class TaskGroupsListComponent extends AbstractTemplateBuilderComponent implements OnInit, OnDestroy {
  ...
  columns: ActionableDynamicTableColumnDefinition[] = [
    ...
    { key: 'type', header: 'Type', property: 'type', type: ActionableDynamicTableColumnType.TEXT },
    {
      ...
      actions: [ActionableDynamicTableActionType.REMOVE],
    },
  ];
  ...
  onChange() {
    this.groups = this.builderService.templateAsObservable().pipe(
      map((t) => {
        if (t) {
          return t.taskGroups;
        } else {
  ...
  public remove = (group: ApiTaskGroup) => {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
        text: 'Are you sure to delete this task group? Deletion of task groups causes associated tasks to be deleted as well.',
  ...
  public addTaskGroup() {
    const dialogRef = this.dialog.open(CompetitionBuilderTaskGroupDialogComponent, {
      data: {
        ...
        group: null,
      } as CompetitionBuilderTaskGroupDialogData,
      width: '600px',
