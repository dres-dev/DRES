...
export class TemplateBuilderComponent
  extends AbstractTemplateBuilderComponent
  implements OnInit, OnDestroy, DeactivationGuarded, TaskTemplateEditorLauncher
{
  onChange() {}
  ...
  ngOnInit(): void {
    ...
    this.routeSub = this.route.params.subscribe((p) => {
      this.templateService.getApiV2TemplateByTemplateId(p.templateId).subscribe(
        (t) => {
  ...
  fileProvider = () => {
    return this.builderService.getTemplate()?.name
      ? this.builderService.getTemplate().name
      : 'evaluation-template-download.json';
  };
  ...
  private openImportDialog(templates: ApiEvaluationTemplate[]) {
    ...
    const dialogref = this.dialg.open(TemplateImportDialogComponent, {
      width: '800px',
      data: { templates: templates, branches: TemplateImportTreeBranch.ALL } as TemplateImportDialogData,
    });
  ...
  public onUpload(contents: any) {
    ...
    if (instanceOfTemplate(contents)) {
      ...
      this.snackBar.open(
        `Uploaded file was either not a JSON file or did not conform to the expected types (EvaluationTemplate or Evaluation)`,
        null,
        { duration: 5000 }
      );
  ...
  public save() {
    if (!this.taskEditor?.isFormValid()) {
      this.dialg.open(InformationDialogComponent, {
        width: '600px',
        data: {
          text: 'There are validation errors on the task builder, please fix them before saving.',
    ...
    this.templateService
      .patchApiV2TemplateByTemplateId(this.builderService.getTemplate().id, this.builderService.getTemplateCleaned())
      .subscribe(
        (s) => {
